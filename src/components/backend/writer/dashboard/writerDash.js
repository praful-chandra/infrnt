...
import { addBlogHead ,addBlogContent,getWriterBlogs,deleteBlog} from "../../../../actions/writerActions";
...
class writerDash extends Component {
  ...
  deleteBlog = blogCode =>{
    this.props.deleteBlog(blogCode);
    const updatedBlogs = this.state.myBlogs.filter(blogs=>{
      if(blogs.blogCode !== blogCode ){
        return blogs
      }else
      return null
    })
  ...
  card = (blog) => (
    <div className="card" style={{ width: "30rem", fontSize: "2rem",margin:"1rem" }}>
      ...
      <div className="card-body">
        ...
        <button onClick={()=>this.deleteBlog(blog.blogCode)} className="card-link btn btn-outline-danger">
          Delete Blog
        </button>
      </div>
    </div>
